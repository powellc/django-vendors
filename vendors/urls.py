...
urlpatterns = patterns('vendors.views',
    ...
    url(r'^edit/(?P<slug>[-\w]+)/$', view=views.vendor_edit, name="vendor_edit"), # Edit your vendor profile
    url(r'^delete/(?P<slug>[-\w]+/$', view=views.vendor_delete, name="vendor_delete"), # Delete your vendor profile
    url(r'^(?P<slug>[-\w]+)/applications/$', view=views.application_index, name="application_index"), # Display a  list of apps for the vendor <slug>
    url(r'^(?P<slug>[-\w]+)/applications/new/$', view=views.application_create, name="application_create"), # Create a new app
    url(r'^(?P<slug>[-\w]+)/applications/edit/(?P<year>\d{4})/$', view=views.application_edit, name="application_edit"), # Edit an application (not sure if it's a good idea...)
    url(r'^(?P<slug>[-\w]+)/applications/delete/(?P<year>\d{4})/$', view=views.application_delete, name="application_delete"), # Delete an application 
